...
Image<RgbaColor> ReadPng(const std::string& file_path) {
  ...
  for (size_t i = 0; i < data.size(); ++i) {
    data[i] = RgbaColor{image[4 * i], image[4 * i + 1], image[4 * i + 2],
                        image[4 * i + 3]};
  }
...
void WritePng(const std::string& file_path, const Image<RgbaColor>& image) {
  ...
  for (size_t i = 0; i < data.size(); ++i) {
    image_data[4 * i + 0] = data[i].r;
    image_data[4 * i + 1] = data[i].g;
    image_data[4 * i + 2] = data[i].b;
    image_data[4 * i + 3] = data[i].a;
  }
